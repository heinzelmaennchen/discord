...
class asdf(commands.Cog):
                ...
                if cursor.rowcount > 0:
                    ...
                    for row in rows:
                        user_max_streak = self.getUserStreak(
                            cursor, True, int(row[0]), resetDate, today)
                        user_active_streak = self.getUserStreak(
                            cursor, False, int(row[0]), resetDate, today)
    ...
    def getUserStreak(self, cursor, isMax, user, resetDate, today):
        # reuse cursor passed from caller
        query = (f"""SELECT c.dt,
                            IFNULL(a.asdf, '0')
                     FROM `calendar` c
                     LEFT OUTER JOIN
                     ...
                     ORDER BY c.dt;""")
        cursor.execute(query)
        # No commit needed for SELECT, and we are sharing cursor
