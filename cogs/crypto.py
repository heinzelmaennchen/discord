...
class crypto(commands.Cog):
  ...
  def getCurrentValues(self, coin, globalStats = False, currency = 'EUR'):
    ...
    apiRequestCoins = requests.get(
      'https://coinlib.io/api/v1/coin?key=' + self.api_key + '&pref=' + currency + '&symbol='
      + coin)
    ...
    if globalStats:
      ...
      'https://coinlib.io/api/v1/global?key=' + self.api_key + '&pref=EUR'
      ).json()
      ...
      rating_24h = self.calculateRating(round(float(apiRequestCoins['coins'][0]['delta_24h']),2))
      rating_7d = self.calculateRating(round(float(apiRequestCoins['coins'][0]['delta_7d']),2))
      rating_30d = self.calculateRating(round(float(apiRequestCoins['coins'][0]['delta_30d']),2))
  ...
  def getTopTenCoins(self):
    topTenList = requests.get(
      'https://coinlib.io/api/v1/coinlist?key=' + self.api_key + '&pref=EUR&page=1&order=rank_asc'
      ).json()
    topTenCoins = []
    for i in range(10):
